...
def generate_links_dictionary(network_size, sim_time, Fault_Per_Second):
	"""
...
#
#----------------------------------------------------------------------------------------------
def generate_fault_injection_do(file_path, sim_time, sim_end, fault_list, Fault_Per_Second):
	"""
	...
	*T->P:  ___|____________|____________|____________||||||||||______________________|''''''''''''''''''''
	...
	if FAULT_GENERATION:
		...
		if Fault_Per_Second > 1000000:
			filename = '/fault_inject' + '_' + str(Fault_Per_Second / 1000000) + 'M.do'
		elif Fault_Per_Second >= 1000:
			filename = '/fault_inject' + '_' + str(Fault_Per_Second / 1000) + 'k.do'
		else:
			filename = '/fault_inject' + '_' + str(Fault_Per_Second) + '.do'
	...
	print path+filename
	fault_inject_file = open(path + filename, 'w')
		...
		if i in temp_dict.keys():
			...
			for item in temp_dict[i]:
				...
				if item.Type == "I" or item.Type == "T->I" or item.Type == "T->P":
					...
					string = "force -drive sim/:"+location+" " + str(random.choice(["0", "1"]))
					random_start = random.randint(0, deviation)+random.randint(0, 10)/10.0
					string +=  " "+str(random_start)+"ns -cancel "+str(random_start+FAULT_LENGTH)+"ns"
